...
namespace FastestEnumerator
{
    ...
    public static class FastEnumerator
    {
        ...
        /// </exception>
        public static IEnumerable<FileData> EnumerateFiles(string path,string searchType)
        {
            return FastEnumerator.EnumerateFiles(path, "*", searchType);
        }
        ...
        /// </exception>
        public static IEnumerable<FileData> EnumerateFiles(string path, string searchPattern, string searchType)
        {
            return FastEnumerator.EnumerateFiles(path, searchPattern, SearchOption.TopDirectoryOnly, searchType);
        }
        ...
        /// </exception>
        public static IEnumerable<FileData> EnumerateFiles(string path, string searchPattern, SearchOption searchOption, string searchType)
        {
            if (path == null)
            {
            ...
            return new FileEnumerable(fullPath, searchPattern, searchOption,searchType);
        }
        ...
        /// <paramref name="filter"/> is a null reference (Nothing in VB)
        /// </exception>
        public static FileData[] GetFiles(string path, string searchPattern, SearchOption searchOption, string searchType)
        {
            IEnumerable<FileData> e = FastEnumerator.EnumerateFiles(path, searchPattern, searchOption, searchType);
        ...
        private class FileEnumerable : IEnumerable<FileData>
        {
            ...
            /// operation should include all subdirectories or only the current directory.
            /// </param>
            public FileEnumerable(string path, string filter, SearchOption searchOption,string searchType)
            {
                m_path = path;
                m_filter = filter;
            ...
            {
                return new FileEnumerator(m_path, m_filter, m_searchOption, m_searchtype);
            }
